...
import iceCream from "@/common/images/iceCream.png";
import circlePlus from "@/common/images/circle-plus.png";
import fastFood from "@/common/images/fast-food.png";

interface IceCreamsProps {
  singleItem: any;
}

const IceCreams = ({ singleItem }: IceCreamsProps) => {
  ...
  return (
    <div id={singleItem.slug} className={`${singleItem.slug}-container mt-10`}>
      ...
      <div className="flex">
        <div className="items-container flex-1 grid grid-cols-2 gap-3">
        ...
        <div className="image flex-1 flex items-center ml-2">
          <Image src={fastFood} alt="fast food image" className="w-full" />
        </div>
      </div>
    </div>
  );
